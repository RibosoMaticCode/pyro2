...
if($mode=="new"){
	...
	if($result['result']){
		...
		if($atributo>0){
			foreach($array_atributo as $atributo){
				...
				$objDataBase->Ejecutar("INSERT INTO blog_posts_posts (nombre_atributo, articulo_id_padre, articulo_id_hijo) VALUES ('".$nombre_atributo."',".$ultimo_id.",".$articulo_id_padre.")");
			}
		}
			if($volver=="true"){
	...
	$objDataBase->Ejecutar("DELETE FROM blog_posts_posts WHERE articulo_id_padre=$id");
	...
	if($atributo>0){
		foreach($array_atributo as $atributo){
			$nombre_atributo = $atributo['nombre'];
			$articulo_id_padre = $atributo['id'];
			$objDataBase->Ejecutar("INSERT INTO blog_posts_posts (nombre_atributo, articulo_id_padre, articulo_id_hijo) VALUES ('".$nombre_atributo."',".$id.",".$articulo_id_padre.")");
		}
	}
