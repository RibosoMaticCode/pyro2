...
var base_module = base_url+'rb-script/modules/suscripciones/';
var base_dashboard = base_url+'rb-admin/';

$(document).ready(function() {

    // Guardar
    $(document).on('submit', '#frmSuscrip', function( event ){
      	event.preventDefault();
  		//tinyMCE.triggerSave(); //save first tinymce en textarea
  		$.ajax({
  			method: "post",
  			url: base_module+"suscrip.save.php",
  			data: $( this ).serialize()
  		})
  		.done(function( data ) {
  			if(data.resultado){
				...
				setTimeout(function(){
	          		window.location.href = base_dashboard+'module.php?pag=rb_sus_susc';
	        	}, 1000);
  	  		}else{
  				notify(data.contenido);
  	  		}
    ...
    $('.del').on("click", function(event){
	  	...
	  	if ( eliminar ) {
		    ...
		    $.ajax({
		      type: "GET",
		      url: base_module+"suscrip.del.php?id="+id
		    })
		    .done(function( data ) {
	      		if(data.resultado){
		        notify(data.contenido);
	...
	$('#suscriptores_config').submit(function(event){
  		...
  		$.ajax({
  			method: "post",
  			url: base_module+"suscrip_config.save.php",
  			data: $( this ).serialize()
  		})
  		.done(function( data ) {
  			if(data.resultado){
				$.fancybox.close();
  				notify(data.contenido);
				setTimeout(function(){
	          		window.location.href = base_dashboard+'module.php?pag=rb_sus_susc_config';
	        	}, 1000);
  	  		}else{
  				notify(data.contenido);
  	  		}
